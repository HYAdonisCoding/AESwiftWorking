...
class AEFormCalenderTCell: AEFormBaseTCell {

    override class func loadCode(tableView: UITableView, index: IndexPath) -> AEFormCalenderTCell {
        let identifier: String = String(describing: AEFormTextFieldTCell.self)
        tableView.register(AEFormCalenderTCell.self, forCellReuseIdentifier: identifier)
        let cell: AEFormCalenderTCell = tableView.dequeueReusableCell(withIdentifier: identifier, for: index as IndexPath) as! AEFormCalenderTCell
